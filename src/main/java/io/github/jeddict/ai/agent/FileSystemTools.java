...
public class FileSystemTools extends AbstractCodeTool {
     ...
     *
     * <ul>
     *   <li>Maven ({@code pom.xml}): {@code mvn[w] exec:java -Dexec.mainClass=&lt;mainClass&gt;}</li>
     *   <li>Gradle ({@code build.gradle[.kts]}): {@code gradle[w] run --main-class=&lt;mainClass&gt;}</li>
     *   <li>Ant ({@code build.xml}): {@code ant run -Dmain.class=&lt;mainClass&gt;}</li>
     *   <li>Fallback: {@code java -cp &lt;jarPath&gt; &lt;mainClass&gt;} using a JAR from {@code target/}</li>
     * </ul>
     *
    ...
    String resolveRunCommand(String mainClass) {
        ...
        final File targetDir = new File(basedirFile, "target");
        if (targetDir.isDirectory()) {
            final File[] jars = targetDir.listFiles((dir, name) ->
                name.endsWith(".jar")
